...
class Render {
  ...
  function renderPixel($x, $y, $z, $r, $g, $b, $texture = null, $u = 0, $v = 0) {
    if ($texture != null) {
      $tmp = $texture->getColorAt($u, $v);
      $r = $tmp->red * ($r / 255);
      $g = $tmp->green * ($g / 255);
      $b = $tmp->blue * ($b / 255);
    }
  ...
  function drawSpan($span, $y, $texture) {
    ...
    $diff_u = $span->__get("u2") - $span->__get("u1");
    $diff_v = $span->__get("v2") - $span->__get("v1");
    $factor = 0;
    ...
    for ($x=$span->__get("x1");$x<$span->__get("x2");$x++) {
      ...
      $u = $span->__get("u1") + $diff_u * $factor;
      $v = $span->__get("v1") + $diff_v * $factor;
      $this->renderPixel($x, $y, $z, $color->red, $color->green, $color->blue, $texture, $u, $v);
      $factor += $factor_step;
    }
  }

  function drawSpansBetweenEdges($e1, $e2, $texture) {
    ...
    $e1_u_diff = $e1->__get("u2") - $e1->__get("u1");
    $e2_u_diff = $e2->__get("u2") - $e2->__get("u1");
    $e1_v_diff = $e1->__get("v2") - $e1->__get("v1");
    $e2_v_diff = $e2->__get("v2") - $e2->__get("v1");
    ...
    for ($y=$e2->__get("y1");$y<$e2->__get("y2");$y++) {
      $span = new Span(
        ...
        , $e1->__get("u1") + $e1_u_diff * $factor_1
        , $e1->__get("v1") + $e1_v_diff * $factor_1
        , $e1->__get("x1") + $e1_x_diff * $factor_1
        , $e1->__get("z1") + $e1_z_diff * $factor_1
        , $e2->__get("c1")->add($e2_c_diff->mult($factor_2))
        , $e2->__get("u1") + $e2_u_diff * $factor_2
        , $e2->__get("v1") + $e2_v_diff * $factor_2
        , $e2->__get("x1") + $e2_x_diff * $factor_2
        , $e2->__get("z1") + $e2_z_diff * $factor_2);
      $this->drawSpan($span, $y, $texture);
      $factor_1 += $factor_1_step;
      $factor_2 += $factor_2_step;
    }
  }

  function renderFace(Vertex $vertex1, Vertex $vertex2, Vertex $vertex3, $texture) {
    $z1 = $vertex1->__get("z");
    $x1 = (($vertex1->__get("x") + 1) * $this->_width / 2) / ($z1 / 100);
    $y1 = (($vertex1->__get("y") + 1) * $this->_height / 2) / ($z1 / 100);
    $c1 = $vertex1->__get("color");
    $u1 = $vertex1->__get("u");
    $v1 = $vertex1->__get("v");
    $z2 = $vertex2->__get("z");
    $x2 = (($vertex2->__get("x") + 1) * $this->_width / 2) / ($z2 / 100);
    $y2 = (($vertex2->__get("y") + 1) * $this->_height / 2) / ($z2 / 100);
    $c2 = $vertex2->__get("color");
    $u2 = $vertex2->__get("u");
    $v2 = $vertex2->__get("v");
    $z3 = $vertex3->__get("z");
    $x3 = (($vertex3->__get("x") + 1) * $this->_width / 2) / ($z3 / 100);
    $y3 = (($vertex3->__get("y") + 1) * $this->_height / 2) / ($z3 / 100);
    $c3 = $vertex3->__get("color");
    $u3 = $vertex3->__get("u");
    $v3 = $vertex3->__get("v");
    $edges = array(new Edge($c1, $u1, $v1, $x1, $y1, $z1, $c2, $u2, $v2, $x2, $y2, $z2)
      , new Edge($c2, $u2, $v2, $x2, $y2, $z2, $c3, $u3, $v3, $x3, $y3, $z3)
      , new Edge($c3, $u3, $v3, $x3, $y3, $z3, $c1, $u1, $v1, $x1, $y1, $z1));
    $max_length = 0;
    $long_edge = 0;
    for ($i=0;$i<3;$i++) {
    ...
    $this->drawSpansBetweenEdges($edges[$long_edge], $edges[$short_edge_1], $texture);
    $this->drawSpansBetweenEdges($edges[$long_edge], $edges[$short_edge_2], $texture);
  }

  function renderTriangle(Triangle $triangle, $render, $texture = null) {
    ...
    elseif ($render == Render::RASTERIZE) {
      $this->renderFace($triangle->__get("v1"), $triangle->__get("v2"), $triangle->__get("v3"), $texture);
    }
  }

  function renderMesh(array $mesh, $render, $texture = null) {
    foreach ($mesh as $triangle) {
      $this->renderTriangle($triangle, $render, $texture);
    }
  }
